...
def siglip_loss(logits):
    """Pairwise sigmoid loss (SigLIP-style), gradients preserved for first row/column only."""
    n      = logits.size(0)
...
def cal_nao_distribution(
    ...
    print(f"✅ Using SigLIP contrastive loss | Encoder: {encoder}")
    ...
    for i, batch_data in outer_pbar:
        ...
        # Compute image-text similarity matrix and apply SigLIP loss
        ...
        for j, inner_batch_data in inner_pbar:
            ...
            inner_image_features = F.normalize(inner_outputs.image_embeds, p=2, dim=1)
            inner_text_features  = F.normalize(inner_outputs.text_embeds,  p=2, dim=1)

            inner_logits = torch.matmul(inner_image_features, inner_text_features.t())
            inner_loss   = criterion(inner_logits)
            inner_loss.backward()
            ...
            for name, param in model.named_parameters():
                if param_keyword in name and param.grad is not None:
                    ...
                    if grad_idx < len(ref_grads):
                        ...
                        # Layer-size-weighted running average (avoids overflow from large score * num)
...
def heat_map(csv_path, labels, save_path):
    ...
    plt.title('Gradient Orthogonality Heatmap - SigLIP (Image-Text Contrastive Loss)')
...
def plot_last_column_from_heatmap(csv_path, labels, save_path):
    ...
    plt.title('Average Gradient Orthogonality - SigLIP (Image-Text Contrastive Loss)')
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Gradient Orthogonality analysis for SigLIP on COCO Captions"
    )
    parser.add_argument('--gpu_id',          type=int, default=0,
                        help='GPU index to use')
    parser.add_argument('--pretrained_path', type=str, default="/home/siglip-base-patch16-224",
    ...
    tag              = f"siglip_{args.encoder}_contrastive"
    ...
    print_stage("Starting", f"Model: SigLIP | Encoder: {args.encoder}")
    device = torch.device(f"cuda:{args.gpu_id}" if torch.cuda.is_available() else "cpu")

    processor = AutoProcessor.from_pretrained(args.pretrained_path)
    model     = AutoModel.from_pretrained(args.pretrained_path)
