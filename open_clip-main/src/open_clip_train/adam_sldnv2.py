    ...
    def _init_group(
        ...
        for p in group["params"]:
            if p.grad is not None:
                ...
                exp_avgs.append(state["exp_avg"])
                exp_avg_sqs.append(state["exp_avg_sq"])
                initial_norms.append(state['init_avg_abs_param'])
...
def _single_tensor_adam_s(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        exp_avg_sq = exp_avg_sqs[i]
        step_t = state_steps[i]
        initial_norm_t = initial_norms[i]
        ...
        if capturable or differentiable:
            ...
            step_size = lr / bias_correction1 * initial_norm_t
            step_size_neg = step_size.neg()
            ...
            step = _get_value(step_t)
            initial_norm = _get_value(initial_norm_t)

            bias_correction1 = 1 - beta1**step
            bias_correction2 = 1 - beta2**step

            step_size = lr / bias_correction1 * initial_norm
...
def _multi_tensor_adam_s(
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, state_steps,initial_norms]  # type: ignore[list-item]
    )
    ...
    for (
        ...
        device_exp_avg_sqs = cast(list[Tensor], device_exp_avg_sqs_)
        device_state_steps = cast(list[Tensor], device_state_steps_)
        device_initial_norms = cast(list[Tensor], device_initial_norms_)
        ...
        if capturable:
            ...
            torch._foreach_add_(exp_avg_sq_sqrt, eps)
            torch._foreach_div_(exp_avg_sq_sqrt, step_size)
            torch._foreach_div_(exp_avg_sq_sqrt, device_initial_norms)
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_adam_s)
def adam_s(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avgs: list[Tensor],
    ...
    *,
    ...
    #     func = _fused_adam_s
    # elif foreach and not torch.jit.is_scripting():
    #     func = _multi_tensor_adam_s
    # else:
    #     func = _single_tensor_adam_s
...
def _single_tensor_adam_wo_mom(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avg_sqs: list[Tensor],
    max_exp_avg_sqs: list[Tensor],
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            if differentiable:
                param.addcdiv_(grad.clone(), denom)
            else:
...
def _multi_tensor_adam_wo_mom(
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avg_sqs, max_exp_avg_sqs, state_steps]  # type: ignore[list-item]
    )
    ...
    for (
        ...
        if capturable:
            ...
            torch._foreach_div_(exp_avg_sq_sqrt, bias_correction2_sqrt)
            torch._foreach_add_(exp_avg_sq_sqrt, eps)
            torch._foreach_div_(exp_avg_sq_sqrt, -lr)

            # at this point, exp_avg_sq_sqrt = - (1 - beta^t) * [sqrt(exp_avg_sq / (1 - beta2^t)) + eps] / lr
            torch._foreach_addcdiv_(device_params, device_grads, exp_avg_sq_sqrt)
        else:
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_adam_wo_mom)
def adam_wo_mom(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avg_sqs: list[Tensor],
    ...
    *,
    ...
    #     func = _single_tensor_adam_s
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_adam_wo_mom
    else:
        func = _single_tensor_adam_wo_mom
        ...
        for p in group["params"]:
            if p.grad is not None:
                ...
                if len(state) == 0:
                    ...
                    state['init_avg_abs_param'] = torch.mean(torch.abs(p.clone()))
                    state['param_num'] = torch.sqrt(p.numel()*torch.ones_like(state['init_avg_abs_param'], memory_format=torch.preserve_format))
                    if state['init_avg_abs_param'] < group['eps']:
                        state['init_avg_abs_param'] = 0.1*init_avg_abs_param_buffer*torch.ones_like(state['init_avg_abs_param'], memory_format=torch.preserve_format)
                    init_avg_abs_param_buffer = state['init_avg_abs_param']
                    if group["amsgrad"]:
                ...
                exp_avg_sqs.append(state["exp_avg_sq"])
                initial_norms.append(state['init_avg_abs_param'])
                param_nums.append(state["param_num"])
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            if amsgrad:
                ...
                # Folds in (admittedly ugly) 1-elem step_size math here to avoid extra param-set-sized read+write
                # (can't fold it into addcdiv_ below because addcdiv_ requires value is a Number, not a Tensor)
                dynamics = torch.div(exp_avg, max_exp_avg_sqs[i].sqrt().add_(eps))
            else:
                dynamics = torch.div(exp_avg, exp_avg_sq.sqrt().add_(eps))
                
            param.addcdiv_(dynamics * step_size_neg, torch.norm(dynamics).div_(param_nums).add_(eps))
            ...
            param.addcdiv_(dynamics, torch.norm(dynamics).div_(param_nums).add_(eps), value=-step_size)
...
def _multi_tensor_adam_s2(
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, state_steps,initial_norms,param_nums]  # type: ignore[list-item]
    )
    ...
    for (
        ...
        device_state_steps = cast(list[Tensor], device_state_steps_)
        device_initial_norms = cast(list[Tensor], device_initial_norms_)
        device_param_nums = cast(list[Tensor], device_param_nums_)
        ...
        if capturable:
            ...
            torch._foreach_add_(exp_avg_sq_sqrt, eps)
            exp_avg_sq_sqrt = torch._foreach_div(device_exp_avgs,exp_avg_sq_sqrt)
            del device_exp_avgs
            
            torch._foreach_mul_(exp_avg_sq_sqrt, -lr)
            torch._foreach_mul_(exp_avg_sq_sqrt, device_initial_norms)
            denom = torch._foreach_norm(exp_avg_sq_sqrt)
            torch._foreach_div_(denom, device_param_nums)
            torch.foreach_add_(denom, eps)
            # at this point, exp_avg_sq_sqrt = - (1 - beta^t) * [sqrt(exp_avg_sq / (1 - beta2^t)) + eps] / lr
            torch._foreach_addcdiv_(device_params, exp_avg_sq_sqrt, denom)
        else:
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_adam_s2)
def adam_s2(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avgs: list[Tensor],
    ...
    *,
    ...
    #     func = _single_tensor_adam_s
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_adam_s2
    else:
        func = _single_tensor_adam_s2
    ...
    def _init_group(
        ...
        for p in group["params"]:
            if p.grad is not None:
                ...
                state_steps.append(state["step"])
        # print([n.item() for n in torch._foreach_norm(initial_norms)])
        # raise ValueError()
        return has_complex
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            denom = (
                torch.norm(exp_avg) / (param_num*step_size_neg*initial_norm)
            ).add_(eps / (step_size_neg*initial_norm))
            ...
            denom = torch.norm(exp_avg).div_(param_num).add_(eps)
            
            param.addcdiv_(exp_avg, denom, value=-step_size*initial_norm)


def _multi_tensor_adam_s_ldn(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avgs: list[Tensor],
    state_steps: list[Tensor],
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avgs, state_steps,initial_norms,param_nums]  # type: ignore[list-item]
    )
    ...
    for (
        ...
        if capturable:
            ...
            denom = torch._foreach_norm(device_exp_avgs)
            torch._foreach_div_(denom, device_param_nums)
            torch._foreach_add_(denom, eps)
            torch._foreach_div_(denom, -lr)
            torch._foreach_div_(denom, device_initial_norms)
            ...
            denom = torch._foreach_norm(device_exp_avgs)
            torch._foreach_div_(denom, device_param_nums)
            # denom = torch._foreach_sqrt(device_exp_avgs)
            torch._foreach_add_(denom, eps)
            # torch._foreach_div_(device_exp_avgs, denom)
            # print([n.item() for n in torch._foreach_norm(denom)])
            # raise ValueError()
            # torch._foreach_mul_(device_initial_norms, -lr)
            # torch._foreach_div_(device_exp_avgs, denom)
            torch._foreach_div_(denom, device_initial_norms)
            # print([n.item() for n in torch._foreach_norm(device_exp_avgs)])
            # torch._foreach_mul_(device_exp_avgs, -lr)
            # print([n.item() for n in torch._foreach_norm(device_exp_avgs)])
            # raise ValueError()
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_adam_s_ldn)
def adam_s_ldn(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avgs: list[Tensor],
    ...
    *,
    ...
    #     func = _single_tensor_adam
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_adam_s_ldn
    else:
        func = _single_tensor_adam_s_ldn
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            denom = (
                torch.norm(exp_avg) / (param_num*step_size_neg)
            ).add_(eps / (step_size_neg))
...
def _multi_tensor_adam_ldn(
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avgs, state_steps,param_nums]  # type: ignore[list-item]
    )
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_adam_ldn)
def adam_ldn(
    params: list[Tensor],
    grads: list[Tensor],
    exp_avgs: list[Tensor],
    ...
    *,
    ...
    #     func = _single_tensor_adam
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_adam_ldn
    else:
        func = _single_tensor_adam_ldn
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            denom = (
                torch.norm(grad) / (param_num*step_size_neg*initial_norm)
            ).add_(eps / (step_size_neg*initial_norm))
            ...
            denom = torch.norm(grad).div_(param_num).add_(eps)
            
            param.addcdiv_(grad, denom, value=-step_size*initial_norm)


def _multi_tensor_sgd_s_ldn(
    params: list[Tensor],
    grads: list[Tensor],
    state_steps: list[Tensor],
    initial_norms: list[Tensor],
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, state_steps,initial_norms,param_nums]  # type: ignore[list-item]
    )
    ...
    for (
        ...
        if capturable:
            ...
            denom = torch._foreach_norm(device_grads)
            torch._foreach_div_(denom, device_param_nums)
            torch._foreach_add_(denom, eps)
            torch._foreach_div_(denom, -lr)
            torch._foreach_div_(denom, device_initial_norms)
            ...
            # at this point, exp_avg_sq_sqrt = - (1 - beta^t) * [sqrt(exp_avg_sq / (1 - beta2^t)) + eps] / lr
            torch._foreach_addcdiv_(device_params, device_grads, denom)
        else:
            step_size = -lr
            denom = torch._foreach_norm(device_grads)
            torch._foreach_div_(denom, device_param_nums)
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_sgd_s_ldn)
def sgd_s_ldn(
    params: list[Tensor],
    grads: list[Tensor],
    state_steps: list[Tensor],
    ...
    *,
    ...
    #     func = _single_tensor_adam
    if foreach and not torch.jit.is_scripting():
        func = _multi_tensor_sgd_s_ldn
    else:
        func = _single_tensor_sgd_s_ldn
